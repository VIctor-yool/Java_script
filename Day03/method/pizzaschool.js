...
//         - 매출에 가격 추가
//         -재고 1 감소
//         - 콘솔에 `"000 피자가 판매되었습니다. 현재 매출: xxxx원"` 출력
//       - 재고가 0개라면:
//         -`"000 피자는 품절입니다."` 출력
//     2. `restock(menuNumber)`
//       - 해당 메뉴 번호의 재고 +1
//       - `"000 피자 재고가 충당되었습니다. 현재 재고: N개"` 출력
...
const arr = [1, 2, 3, 4, 5];
arr.slice(0, 3); //3번째 포함x
arr[2]; // 3

console.log(school.menus[0].price); // 7001

const school = {
  sales: 0,
  menus: [
    { name: "치즈피자", price: 7001, stock: 3 },
    { name: "불고기피자", price: 7000, stock: 3 },
  ...
  order: function (menuNumber) {
    if ((this, menus[menuNumber - 1].stock > 0)) {
      ...
      console.log(
        `${this.menus[menuNumber - 1]} 피자가 판매되었습니다. 현재 매출: ${
          this.sales
        }원`
      );
    } else {
